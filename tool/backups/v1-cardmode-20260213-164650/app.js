(() => {
  ...
  async function hostChangeBanker() {
    ...
    try {
      const bankerId = dom.hostBankerSelect.value;
  ...
  function renderRoomMeta() {
    ...
    dom.roomMeta.textContent = `房间 ${ctx.state.roomId} | 用户名：${userName} | 第 ${ctx.state.round.seq} 局 | 你的身份：${role} | ${ctx.state.isHost ? "你是房主" : ""}`;
  }
